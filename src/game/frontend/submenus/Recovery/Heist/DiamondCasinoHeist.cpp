...
namespace YimMenu::Submenus
{
	std::shared_ptr<TabItem> RenderDiamondCasinoHeistMenu()
	{
		...
		auto setups = std::make_shared<Group>("Heist Setups", 9);
		...
		setups->AddItem(std::make_shared<SameLineItem>(std::make_shared<ListCommandItem>("diamondcasinoheistweapon"_J)));
		setups->AddItem(std::make_shared<ListCommandItem>("diamondcasinoheistdriver"_J));
		setups->AddItem(std::make_shared<SameLineItem>(std::make_shared<ListCommandItem>("diamondcasinoheistvehicle"_J)));
		...
		loots->AddItem(std::make_shared<SameLineItem>(std::make_shared<CommandItem>("diamondcasinoheistsetpotentialtake"_J, "Set##potentialtake")));
		loots->AddItem(std::make_shared<IntCommandItem>("diamondcasinoheistactualtake"_J));
		loots->AddItem(std::make_shared<SameLineItem>(std::make_shared<CommandItem>("diamondcasinoheistsetactualtake"_J, "Set##actualtake")));
